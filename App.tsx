...
type View = 'dashboard' | 'sales' | 'sales-list' | 'customers' | 'products' | 'reports' | 'settings';


const NavItem: React.FC<{ icon: React.ReactNode; label: string; isActive: boolean; onClick: () => void; }> = ({ icon, label, isActive, onClick }) => (
    <button
        onClick={onClick}
        className={`flex flex-col items-center justify-center space-y-1 w-full py-2 px-1 text-xs md:flex-row md:justify-start md:space-y-0 md:space-x-3 md:px-4 md:py-3 md:text-sm rounded-lg transition-colors ${isActive ? 'bg-indigo-600 text-white' : 'text-gray-400 hover:bg-gray-700 hover:text-white'
            }`}
    >
        {icon}
        <span>{label}</span>
...
const AppLayout: React.FC = () => {
    ...
    const renderView = () => {
        switch (currentView) {
            ...
            case 'sales-list': return <SalesList />;
        ...
        <div className="flex flex-col md:flex-row h-screen font-sans">
            <nav className="bg-gray-800 w-full md:w-56 p-2 md:p-4 order-last md:order-first flex md:flex-col justify-around md:justify-start">
                ...
                <div className="flex-grow flex md:flex-col justify-around md:justify-start md:space-y-2">
                    ...
                    <NavItem icon={<BanknotesIcon className="w-6 h-6" />} label="Atendimento" isActive={currentView === 'sales'} onClick={() => setCurrentView('sales')} />
                    <NavItem icon={<ShoppingCartIcon className="w-6 h-6" />} label="Vendas" isActive={currentView === 'sales-list'} onClick={() => setCurrentView('sales-list')} />
